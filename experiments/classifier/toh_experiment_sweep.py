...
if __name__ == "__main__":
    ...
    all_datasets = [d for d in os.listdir(base_dataset_path) if os.path.isdir(os.path.join(base_dataset_path, d))]
    
    if len(all_datasets) < 11:
        raise ValueError(f"Need at least 11 datasets in {base_dataset_path}, found {len(all_datasets)}")

    # Shuffle and split datasets
    random.shuffle(all_datasets)
    train_set_name = all_datasets[0]
    unlabeled_set_name = all_datasets[1:5]
    test_set_names = all_datasets[5:11]

    print(f"Selected Train Set: {train_set_name}")
    print(f"Selected Unlabeled Sets: {unlabeled_set_name}")
    print(f"Selected Test Sets: {test_set_names}")

    # Find a json file in the training set to determine the object of interest
    train_set_path = os.path.join(base_dataset_path, train_set_name)
    try:
        # Find the first json file to inspect state
        first_json_path = glob.glob(os.path.join(train_set_path, 'images', '*.json'))[0]
        with open(first_json_path, 'r') as f:
            json_dict = json.load(f)
        state_dict = json_dict['object_states'] # nested dict [stick: dict for levels (level: objects)]

        # Collect all possible objects from all sticks
        possible_objects = set()
        for stick in ['left', 'middle', 'right']:
            if stick in state_dict:
                for obj in state_dict[stick].values():
                    # Assuming object representation is hashable (like a tuple or string)
                    # If it's a list, convert to tuple
                    if isinstance(obj, list):
                        possible_objects.add(tuple(obj))
                    else:
                        # Handle other potential types if necessary, ensure hashable
                        possible_objects.add(obj) 
        
        if not possible_objects:
             raise ValueError(f"No objects found in state_dict of {first_json_path}")

        # Randomly select one object of interest
        selected_object = random.choice(list(possible_objects))
        # Convert back to list if it was originally a list
        object_of_interest = list(selected_object) if isinstance(selected_object, tuple) else selected_object

        print(f"Randomly selected Object of Interest: {object_of_interest}")

    except IndexError:
        raise FileNotFoundError(f"No JSON files found in {train_set_path}/images to determine object of interest.")
    except KeyError:
        raise KeyError(f"Key 'object_states' not found in {first_json_path}.")


    skill_of_interest = ['left'] # (stick) - Assuming we always check the left stick based on original code

    # Note: The original code's label_data function checks if the object_of_interest
    # is present *anywhere* on the specified stick (skill_of_interest[0]).
    # The level part (skill_of_interest[1]) seemed unused in the provided label_data function.
    # Kept skill_of_interest definition simple based on this observation. Adjust if needed.
